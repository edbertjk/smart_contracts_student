import { Canister, query, text, Record, update, nat64, Vec,
Result,
StableBTreeMap,
ic,
bool} from 'azle';
...
export default Canister({
    ...
    // createStudent: update([text], Result(student, text), (newMessage) => {
    //     message = newMessage; // This change will be persisted
    // }),
    createSemester: update([text, nat64], Result(semester, text), (id,sks) => {
        try{
        if(!id || !sks){
            throw new Error("Id/Sks must be added");
        }
      const newSemester: Semester = {
        idSemester:id,
        semesterCreditSemester: sks,
        createdAt:ic.time(),
        updatedAt:ic.time()
      };
  
      semesterDatabase.insert(id, newSemester);
      return Result.Ok(newSemester);
    }catch(err){
        return Result.Err("Error Creating Semester [" + err +"]");
    }
    }),
...
return Result.Err("Error Payment [" + err+ "]");
    ...
    addCourseStudent:update([text,text], Result(student, text), (idUser,idCourse) => {
        try{
          ...
          if(!idUser || !idCourse || dataStudent["semesterCreditSemesterTotal"] > 25 || dataStudent["alreadyPay"] == true){
    ...
    if(dataSemester["semesterCreditSemester"] < dataStudent["semesterCreditSemesterTotal"]){
         dataStudent["paymentStudent"] += 1000000n;
    }
